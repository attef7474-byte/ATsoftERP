...
export class MachineSparePartsController {
  ...
  create(@Body() dto: CreateMachineSparePartDto, @CurrentUser('sub') userId: string, @CurrentActiveContext() ctx: ActiveOperationalContext) { return this.service.create(dto, userId, ctx); }

  @Get()
  @Permissions('machine-spare-part:read')
  @ApiOperation({ summary: 'List machine spare part links' })
  findAll(@Query() query: { page?: string; limit?: string; machineId?: string; sparePartId?: string; isPrimary?: string; status?: string }, @CurrentActiveContext() ctx: ActiveOperationalContext) {
  ...
  update(@Param('id') id: string, @Body() dto: UpdateMachineSparePartDto, @CurrentUser('sub') userId: string, @CurrentActiveContext() ctx: ActiveOperationalContext) { return this.service.update(id, dto, userId, ctx); }
