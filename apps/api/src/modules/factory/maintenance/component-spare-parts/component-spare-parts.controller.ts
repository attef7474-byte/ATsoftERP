...
export class ComponentSparePartsController {
  ...
  create(@Body() dto: CreateComponentSparePartDto, @CurrentUser('sub') userId: string, @CurrentActiveContext() ctx: ActiveOperationalContext) { return this.service.create(dto, userId, ctx); }

  @Get()
  @Permissions('component-spare-part:read')
  @ApiOperation({ summary: 'List component spare part links' })
  findAll(@Query() query: { page?: string; limit?: string; componentId?: string; sparePartId?: string; isPrimary?: string; status?: string }, @CurrentActiveContext() ctx: ActiveOperationalContext) {
  ...
  update(@Param('id') id: string, @Body() dto: UpdateComponentSparePartDto, @CurrentUser('sub') userId: string, @CurrentActiveContext() ctx: ActiveOperationalContext) { return this.service.update(id, dto, userId, ctx); }

  @Patch(':id/deactivate')
  @Permissions('component-spare-part:deactivate')
  @ApiOperation({ summary: 'Deactivate component spare part link' })
  deactivate(@Param('id') id: string, @CurrentUser('sub') userId: string, @CurrentActiveContext() ctx: ActiveOperationalContext) { return this.service.deactivate(id, userId, ctx); }
}
