...
describe('Phase 1.4 permission keys', () => {
  it('matches the controller workflow contract without later-phase actions', () => {
    expect([...PRODUCTION_ORDER_PERMISSION_KEYS].sort()).toEqual([
      ...
      'production-order:create', 'production-order:delete', 'production-order:plan',
      'production-order:read', 'production-order:readiness', 'production-order:recalculate',
      'production-order:release', 'production-order:update',
    ].sort());
