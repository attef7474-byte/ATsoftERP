    ...
    <div className="p-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? t('common.edit') : t('common.new')}>
        <div className="space-y-4">
          ...
          <div>
            <F9Lookup label={`${t('maintenance.personnel')} *`} value={form.maintenancePersonnelId} onChange={(v) => { setForm({ ...form, maintenancePersonnelId: v }); setValidationErrors(prev => ({ ...prev, maintenancePersonnelId: '' })); }} adapter={maintenancePersonnelAdapter} />
            {validationErrors.maintenancePersonnelId && <p className="text-red-500 text-sm mt-1">{validationErrors.maintenancePersonnelId}</p>}
          </div>
          <div>
            <Input label={`${t('maintenance.responsibilityRole')} *`} value={form.responsibilityRole} onChange={(e) => { setForm({ ...form, responsibilityRole: e.target.value }); setValidationErrors(prev => ({ ...prev, responsibilityRole: '' })); }} />
