...
export default function ProductionLinesPage() {
  ...
  const handleSave = async () => {
    ...
    if (!form.branchId) errors.branchId = t('validation.required');
    if (!form.departmentId) errors.departmentId = t('validation.required');
    if (!form.operationTypeId) errors.operationTypeId = t('validation.required');
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? t('maintenance.editProductionLine') : t('maintenance.newProductionLine')} size="lg">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Select label={t('core.department')} value={form.departmentId} placeholder={t('common.select')} options={departments.map((d: any) => ({ value: d.id, label: d.name }))} onChange={(e) => { setForm({ ...form, departmentId: e.target.value }); setValidationErrors(prev => ({ ...prev, departmentId: '' })); }} />
              {validationErrors.departmentId && <p className="text-red-500 text-sm mt-1">{validationErrors.departmentId}</p>}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Select label={t('maintenance.operationType')} value={form.operationTypeId} placeholder={t('common.select')} options={operationTypes.map((o: any) => ({ value: o.id, label: o.name }))} onChange={(e) => { setForm({ ...form, operationTypeId: e.target.value }); setValidationErrors(prev => ({ ...prev, operationTypeId: '' })); }} />
              {validationErrors.operationTypeId && <p className="text-red-500 text-sm mt-1">{validationErrors.operationTypeId}</p>}
            </div>
