      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? t('common.edit') : t('common.new')}>
        {loadingDetail ? (
          ...
          <div className="space-y-4">
            ...
            <div>
              <Input label={`${t('maintenance.personnelName')} *`} value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); setValidationErrors(prev => ({ ...prev, name: '' })); }} />
              {validationErrors.name && <p className="text-red-500 text-sm mt-1">{validationErrors.name}</p>}
            </div>
            <div>
              <Input label={`${t('maintenance.personnelRole')} *`} value={form.role} onChange={(e) => { setForm({ ...form, role: e.target.value }); setValidationErrors(prev => ({ ...prev, role: '' })); }} />
