...
export default function SlaEscalatedPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await api.get<{ data: MaintenanceRequest[] }>('/maintenance/dashboard/sla-escalated');
      setData(res.data || []);
    } catch (err: any) {
  ...
  const columns = [
    ...
    { key: 'escalationLevel', header: t('maintenance.escalated'), render: (r: MaintenanceRequest) => <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">{(r as any).escalationLevel}</span> },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('maintenanceDashboard.slaEscalated') || 'SLA Escalated'} />
      <Card>
        <CardHeader><h2 className="text-lg font-semibold">{t('maintenanceDashboard.slaEscalated') || 'SLA Escalated Requests'}</h2></CardHeader>
        <CardContent>
