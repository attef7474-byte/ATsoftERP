...
export default function MachineCategoriesPage() {
  ...
  const [parents, setParents] = useState<MachineCategory[]>([]);
  ...
  const fetchParents = async () => {
    try {
      const res = await api.get<{ data: MachineCategory[] }>('/maintenance/machine-categories', { params: { limit: 50 } });
      setParents(res.data || []);
    } catch {}
  };
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      await api.patch(`/maintenance/machine-categories/${selectedId}`, { status });
  ...
  const columns = [
    ...
    { key: 'machines', header: t('maintenance.machines'), render: (c: MachineCategory) => c._count?.machines ?? '-' },
    { key: 'status', header: t('common.status'), render: (c: MachineCategory) => <StatusBadge status={c.status} /> },
    {
  ...
  return (
    <div>
      ...
      {!error && !loading && data.length > 0 && (
        <Card>
          <DataTable columns={columns} data={data} keyExtractor={(c: MachineCategory) => c.id} />
          <Pagination page={meta.page} totalPages={meta.totalPages} total={meta.total} onPageChange={fetchData} />
        </Card>
      )}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? t('maintenance.editMachineCategory') : t('maintenance.newMachineCategory')}>
        <div className="space-y-4">
          ...
          <Select label={t('maintenance.parentCategory')} value={form.parentId} onChange={(e) => setForm({ ...form, parentId: e.target.value })}
