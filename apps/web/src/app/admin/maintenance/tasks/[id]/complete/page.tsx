...
import { Button, Textarea, Card, CardContent, LoadingState, ErrorState } from '../../../../../../components/admin/ui';
...
export default function CompleteMaintenanceTaskPage() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      await api.patch(`/maintenance/tasks/${id}/complete`, payload);
      ...
      <Card>
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-lg font-semibold text-gray-900">{t('maintenance.completeTask')}</h1>
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('maintenance.assignedTo')}</dt>
              <dd className="mt-1 text-sm text-gray-900">{data.assignedTo?.name || '-'}</dd>
            </div>
            <div>
            ...
            <div>
              <dt className="text-sm font-medium text-gray-500">{t('maintenance.startedAt')}</dt>
              <dd className="mt-1 text-sm text-gray-900">{data.startedAt ? new Date(data.startedAt).toLocaleString() : '-'}</dd>
            </div>
          </dl>

          <div className="space-y-4">
            <Textarea label={t('maintenance.completionNotes')} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t('maintenance.completionNotesPlaceholder')} />
            <div className="flex justify-end gap-3 pt-4">
              <Button variant="secondary" onClick={() => router.back()}>{t('actions.cancel')}</Button>
              <Button onClick={handleComplete} loading={saving}>{t('maintenance.complete')}</Button>
            </div>
          </div>
        </CardContent>
      </Card>
