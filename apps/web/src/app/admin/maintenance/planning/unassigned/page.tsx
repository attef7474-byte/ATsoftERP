...
export default function UnassignedPage() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    setError(null);
    try {
      const res = await api.get<any>('/maintenance/calendar-workload/unassigned', { params: { page, limit: 10 } });
      setData(res.data || []);
      setMeta(res.meta);
    } catch (err: any) {
  ...
  const columns = [
    ...
    { key: 'priority', header: t('maintenance.priority'), render: (r: any) => <StatusBadge status={r.priority} /> },
    { key: 'type', header: t('maintenance.type') },
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">{t('maintenance.unassignedWork')}</h1>
      <Card><CardContent>
