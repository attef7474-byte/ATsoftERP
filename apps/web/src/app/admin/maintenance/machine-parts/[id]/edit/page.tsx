...
export default function EditMachinePartPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setForm({
        ...
        unit: item.unit || '',
        unitPrice: extra.unitPrice != null ? String(extra.unitPrice) : '',
        stockQuantity: extra.stockQuantity ?? 0,
        minimumStock: extra.minimumStock ?? 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.unit !== (data?.unit || '')) payload.unit = form.unit || null;
      const extra = data as any;
      if (form.unitPrice !== (extra.unitPrice != null ? String(extra.unitPrice) : '')) payload.unitPrice = form.unitPrice ? parseFloat(form.unitPrice) : null;
      if (form.stockQuantity !== (extra.stockQuantity ?? 0)) payload.stockQuantity = form.stockQuantity;
      if (form.minimumStock !== (extra.minimumStock ?? 0)) payload.minimumStock = form.minimumStock;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label={t('maintenance.unit')} value={form.unit} onChange={(e) => setField('unit', e.target.value)} disabled={isReadOnly} />
            <Input label={t('maintenance.unitPrice')} type="number" value={form.unitPrice} onChange={(e) => setField('unitPrice', e.target.value)} disabled={isReadOnly} />
            <Input label={t('maintenance.stockQuantity')} type="number" value={String(form.stockQuantity)} onChange={(e) => setField('stockQuantity', parseInt(e.target.value) || 0)} disabled={isReadOnly} />
            <Input label={t('maintenance.minimumStock')} type="number" value={String(form.minimumStock)} onChange={(e) => setField('minimumStock', parseInt(e.target.value) || 0)} disabled={isReadOnly} />
          </div>
        </CardContent>
      </Card>
