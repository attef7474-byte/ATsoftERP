...
export default function CreateLocationPage() {
  ...
  const [form, setForm] = useState({ warehouseId: '', name: '', barcode: '' });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload: any = { warehouseId: form.warehouseId, name: form.name.trim() };
  ...
  const { exec } = useStableHandlers({
    ...
    refresh: () => { setForm({ warehouseId: '', name: '', barcode: '' }); setErrors({}); setDirty(false); },
    save: () => handleSave(),
