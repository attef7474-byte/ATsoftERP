...
export default function InventoryAdjustmentDetailPage() {
  ...
  const execWorkflow = async (action: string) => {
    ...
    try {
      await api.patch(`/inventory/adjustments/${id}/${action}`, {});
  ...
  useRegisterAdminActions([
    ...
    { id: 'post', labelKey: 'inventoryCounting.postAdjustment', icon: <ActionPostIcon />, onClick: () => exec('post'), enabled: !!(data && data.status === 'DRAFT') },
    { id: 'cancel', labelKey: 'inventoryCounting.cancelAdjustment', icon: <ActionCancelIcon />, onClick: () => exec('cancel'), enabled: !!(data && data.status === 'DRAFT'), variant: 'danger' },
  ]);
  ...
  const tabs = [
    ...
    { id: 'lines', label: t('details.inventoryAdjustment.lines') },
  ];
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardContent>
          <dl className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div><dt className="text-sm font-medium text-gray-500">{t('details.inventoryAdjustment.adjustmentNumber')}</dt><dd className="mt-1 text-sm text-gray-900 font-semibold">{data.adjustmentNumber}</dd></div>
            ...
            <div><dt className="text-sm font-medium text-gray-500">{t('details.inventoryAdjustment.reason')}</dt><dd className="mt-1 text-sm text-gray-900">{data.reason || '-'}</dd></div>
      ...
      {activeTab === 'lines' && (
        <Card>
          <CardHeader><h3 className="text-sm font-semibold text-gray-700">{t('details.inventoryAdjustment.lines')}</h3></CardHeader>
          <CardContent>
            {!data.lines || data.lines.length === 0 ? <p className="text-sm text-gray-500 py-4">{t('common.noData')}</p> : (
              <DataTable columns={[
                { key: 'product', header: t('inventoryCounting.product'), render: (l: InventoryAdjustmentLine) => l.product?.code + ' - ' + (l.product?.name || '') || '-' },
                { key: 'location', header: t('inventoryCounting.warehouseLocation'), render: (l: InventoryAdjustmentLine) => l.warehouseLocation?.name || '-' },
                { key: 'systemQty', header: t('inventoryCounting.systemQty'), render: (l: InventoryAdjustmentLine) => l.systemQty },
                { key: 'countedQty', header: t('inventoryCounting.countedQty'), render: (l: InventoryAdjustmentLine) => l.countedQty },
                { key: 'differenceQty', header: t('inventoryCounting.differenceQty'), render: (l: InventoryAdjustmentLine) => l.differenceQty },
                { key: 'notes', header: t('inventoryCounting.notes'), render: (l: InventoryAdjustmentLine) => l.notes || '-' },
              ]} data={data.lines} keyExtractor={(l: InventoryAdjustmentLine) => l.id} />
            )}
          </CardContent>
        </Card>
      )}
