...
export default function EditInventoryAdjustmentPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: any = {};
      if (form.reason !== (data?.reason || '')) payload.reason = form.reason.trim() || null;
      if (form.notes !== (data?.notes || '')) payload.notes = form.notes.trim() || null;
      await api.patch(`/inventory/adjustments/${id}`, payload);
      showToast(t('complexForms.recordUpdated'), 'success');
      router.push(`/admin/inventory/adjustments/${id}`);
    } catch (err: any) {
      ...
      <Card>
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-lg font-semibold text-gray-900">{t('inventoryCounting.editAdjustment')}</h1>
              <p className="text-sm text-gray-500">[{data.adjustmentNumber}]</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
              ...
              <div><span className="font-medium">{t('inventoryCounting.adjustmentDate')}:</span> {data.adjustmentDate ? new Date(data.adjustmentDate).toLocaleDateString() : '-'}</div>
              <div><span className="font-medium">{t('inventoryCounting.reason')}:</span> {data.reason || '-'}</div>
            </div>
            <Input label={t('inventoryCounting.reason')} value={form.reason} onChange={(e) => setField('reason', e.target.value)} disabled={isReadOnly} />
