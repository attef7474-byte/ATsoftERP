...
export default function ProductLabelPrintPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('barcodes.print.productTitle')} subtitle={t('barcodes.print.productSubtitle')} />
      <Card>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <F9Lookup
              label={t('barcodes.print.selectProduct')}
              value={productId}
              onChange={(v) => { setProductId(v); setSelectedLabelId(''); }}
              adapter={productAdapter}
              placeholder={t('barcodes.print.productPlaceholder')}
            />
      ...
      {!loading && productId && labels.length === 0 && (
        <EmptyState message={t('barcodes.print.noLabelsForProduct')} />
      )}
