...
export default function BarcodeScanPage() {
  ...
  const fetchScans = useCallback(async () => {
    ...
    try {
      const res = await api.get<{ data: any[]; meta: any }>('/barcodes/scans', { params: { limit: '20' } });
      setScans(res.data || []);
    } catch {
      setScans([]);
    } finally {
  ...
  const handleScan = useCallback(async () => {
    ...
    setScanError('');
    try {
      const result = await api.post<any>('/barcodes/scan', { value: scanValue.trim(), purpose });
  ...
  useRegisterAdminActions([
    { id: 'refresh', labelKey: 'common.refresh', icon: <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>, onClick: fetchScans },
    { id: 'scanAction', labelKey: 'barcodes.scan.scan', icon: <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>, onClick: handleScan, enabled: !!scanValue.trim() },
    { id: 'clear', labelKey: 'barcodes.scan.clear', icon: <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>, onClick: handleClear },
  ]);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <input ref={scanInputRef} type="text" value={scanValue} onChange={(e) => setScanValue(e.target.value)} onKeyDown={handleKeyDown}
      ...
      {scanResult && (
        <Card>
          <CardHeader><h3 className="text-lg font-semibold text-green-700">{t('barcodes.scan.success')}</h3></CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">{t('barcodes.scannedValue')}</p>
                <p className="font-mono font-bold">{scanResult.scannedValue || scanValue}</p>
              </div>
              {scanResult.label && (
                <>
