...
export default function MachineCardDesignerPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      const res = await api.get<{ data: BarcodeLabelTemplate[] }>('/barcodes/templates', { params: { entityType: 'MACHINE' } });
  ...
  useRegisterAdminActions([
    {
      ...
      onClick: handleGenerate, enabled: !!machineId,
    },
  ]);

  return (
    <div className="space-y-6">
      <PageHeader title={t('barcodes.designer.machineTitle')} subtitle={t('barcodes.designer.machineSubtitle')} />
      <Card>
        <CardContent>
          ...
          <div className="flex gap-3 mt-4">
            <Button onClick={handleGenerate} loading={generating} disabled={!machineId}>{t('barcodes.designer.generate')}</Button>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="border rounded-lg p-6 bg-gray-50 max-w-md mx-auto">
              <div className="text-center">
                ...
                <div className="mt-4 flex justify-center gap-2">
                  ...
                  <Link href={`/admin/barcodes/machine-cards/preview?labelId=${generatedLabel.id}`}>
