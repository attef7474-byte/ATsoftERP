...
import { Button, Card, CardContent, CardHeader, PageHeader, LoadingState, EmptyState, StatusBadge } from '../../../../components/admin/ui';
...
export default function MachineCardsPage() {
  ...
  const fetchLabels = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, any> = { entityType: 'MACHINE', limit: 50, page: 1 };
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('barcodes.machineCards.title')} subtitle={t('barcodes.machineCards.subtitle')} />
      <Card>
            ...
            <div className="flex flex-wrap gap-3 items-end">
              <div className="relative flex-1 min-w-[200px] max-w-sm">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                  placeholder={t('barcodes.machineCards.search')}
              ...
              <Link href="/admin/barcodes/machine-cards/designer">
                <Button>{t('barcodes.machineCards.designNew')}</Button>
              </Link>
            </div>
          </div>
          {loading && <LoadingState message={t('common.loading')} />}
          {!loading && labels.length === 0 && <EmptyState message={t('barcodes.machineCards.noLabels')} />}
                    ...
                    <tr key={label.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <Link href={`/admin/barcodes/machine-cards/preview?labelId=${label.id}`}>
                            <Button variant="ghost" size="sm">{t('common.preview')}</Button>
                          </Link>
                          <Link href={`/admin/barcodes/machine-cards/print?labelId=${label.id}`}>
                            <Button variant="ghost" size="sm">{t('barcodes.print.print')}</Button>
                          </Link>
                          <Link href={`/admin/barcodes/machine-cards/designer?labelId=${label.id}`}>
