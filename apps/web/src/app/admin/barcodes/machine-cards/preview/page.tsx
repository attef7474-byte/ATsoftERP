...
export default function MachineCardPreviewPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('barcodes.preview.machineTitle')} subtitle={t('barcodes.preview.machineSubtitle')} />
      <Card>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <F9Lookup
              label={t('barcodes.preview.selectMachine')}
              value={machineId}
              onChange={(v) => { setMachineId(v); setSelectedLabelId(''); setLabelDetail(null); }}
              adapter={machineAdapter}
              placeholder={t('barcodes.preview.machinePlaceholder')}
            />
      ...
      {!loading && !detailLoading && !selectedLabel && machineId && (
