import type { LocaleTranslations } from '../../types';

const validation: Pick<LocaleTranslations, 'validation' | 'errors' | 'complexForms'> = {
    validation: {
        required: 'هذا الحقل مطلوب.',
        email: 'يرجى إدخال بريد إلكتروني صحيح.',
        minLength: 'يجب أن يكون على الأقل {min} أحرف.',
        maxLength: 'يجب أن يكون على الأكثر {max} أحرف.',
        min: 'يجب أن يكون على الأقل {min}.',
        max: 'يجب أن يكون على الأكثر {max}.',
        number: 'يرجى إدخال رقم صحيح.',
        pattern: 'تنسيق غير صحيح.',
        unique: 'يجب أن تكون هذه القيمة فريدة.',
        duplicateValue: 'قيمة مكررة، يرجى اختيار قيمة مختلفة.',
        invalidReference: 'المرجع المحدد غير صالح أو غير موجود.',
        invalidEnum: 'قيمة غير صالحة لهذا الحقل.',
        invalidDate: 'تاريخ غير صالح.',
        invalidId: 'معرف غير صالح.',
        invalidQuantity: 'كمية غير صالحة.',
        invalidStatusTransition: 'هذه العملية غير مسموح بها في الحالة الحالية.',
        leadershipAdministrationRequired: 'الدور القيادي يتطلب تحديد الإدارة',
        leadershipDepartmentRequired: 'الدور القيادي يتطلب تحديد القسم',
        insufficientStock: 'مخزون غير كافٍ لهذا الصنف.',
        unknownField: 'هذا الحقل غير مسموح به.',
        tooLong: 'القيمة طويلة جداً.',
        tooShort: 'القيمة قصيرة جداً.',
        passwordMismatch: 'تأكيد كلمة المرور غير مطابق.',
        passwordWhitespaceOnly: 'لا يمكن أن تتكون كلمة المرور من مسافات فقط.',
        passwordMinLength: 'يجب ألا يقل طول كلمة المرور عن {min} أحرف.',
        passwordTooLong: 'كلمة المرور أطول من حد bcrypt الآمن ({maxBytes} بايت).',
        passwordUppercase: 'يجب أن تحتوي كلمة المرور على حرف كبير واحد على الأقل.',
        passwordLowercase: 'يجب أن تحتوي كلمة المرور على حرف صغير واحد على الأقل.',
        passwordNumber: 'يجب أن تحتوي كلمة المرور على رقم واحد على الأقل.',
        passwordSymbol: 'يجب أن تحتوي كلمة المرور على رمز واحد على الأقل.',
    },
    errors: {
        loadFailed: 'فشل تحميل البيانات. يرجى المحاولة مرة أخرى.',
        createFailed: 'فشل الإنشاء. يرجى المحاولة مرة أخرى.',
        updateFailed: 'فشل التحديث. يرجى المحاولة مرة أخرى.',
        deleteFailed: 'فشل الحذف. يرجى المحاولة مرة أخرى.',
        serverError: 'خطأ في الخادم. يرجى الاتصال بالمسؤول.',
        networkError: 'خطأ في الشبكة. يرجى التحقق من اتصالك.',
        badRequest: 'طلب غير صالح.',
        notFound: 'المورد غير موجود.',
        forbidden: 'ليس لديك صلاحية لتنفيذ هذا الإجراء.',
        unauthorized: 'يرجى تسجيل الدخول للمتابعة.',
        conflict: 'حدث تعارض في البيانات. يرجى التحديث والمحاولة مرة أخرى.',
        tooLarge: 'الطلب كبير جداً.',
        tooManyRequests: 'طلبات كثيرة جداً. يرجى المحاولة لاحقاً.',
        validationFailed: 'يرجى التحقق من النموذج للأخطاء.',
        generalError: 'حدث خطأ غير متوقع.',
        unavailable: 'تعذر عرض النص المطلوب.',
    },
    complexForms: {
        create: '\u0625\u0646\u0634\u0627\u0621',
        edit: '\u062a\u0639\u062f\u064a\u0644',
        createProduct: '\u0625\u0646\u0634\u0627\u0621 \u0645\u0646\u062a\u062c',
        editProduct: '\u062a\u0639\u062f\u064a\u0644 \u0645\u0646\u062a\u062c',
        createMachine: '\u0625\u0646\u0634\u0627\u0621 \u0645\u0627\u0643\u064a\u0646\u0629',
        editMachine: '\u062a\u0639\u062f\u064a\u0644 \u0645\u0627\u0643\u064a\u0646\u0629',
        createMaintenanceRequest: '\u0625\u0646\u0634\u0627\u0621 \u0637\u0644\u0628 \u0635\u064a\u0627\u0646\u0629',
        editMaintenanceRequest: '\u062a\u0639\u062f\u064a\u0644 \u0637\u0644\u0628 \u0635\u064a\u0627\u0646\u0629',
        createInventoryCount: '\u0625\u0646\u0634\u0627\u0621 \u062c\u0631\u062f \u0645\u062e\u0632\u0648\u0646',
        editInventoryCount: '\u062a\u0639\u062f\u064a\u0644 \u062c\u0631\u062f \u0645\u062e\u0632\u0648\u0646',
        createDowntimeLog: '\u062a\u0633\u062c\u064a\u0644 \u062a\u0648\u0642\u0641',
        editDowntimeLog: '\u062a\u0639\u062f\u064a\u0644 \u062a\u0633\u062c\u064a\u0644 \u062a\u0648\u0642\u0641',
        basicInformation: '\u0645\u0639\u0644\u0648\u0645\u0627\u062a \u0623\u0633\u0627\u0633\u064a\u0629',
        classification: '\u062a\u0635\u0646\u064a\u0641',
        organization: '\u062a\u0646\u0638\u064a\u0645',
        technicalInformation: '\u0645\u0639\u0644\u0648\u0645\u0627\u062a \u0641\u0646\u064a\u0629',
        requestInformation: '\u0645\u0639\u0644\u0648\u0645\u0627\u062a \u0627\u0644\u0637\u0644\u0628',
        assignment: '\u062a\u0639\u064a\u064a\u0646',
        countHeader: '\u0631\u0623\u0633 \u0627\u0644\u062c\u0631\u062f',
        countScope: '\u0646\u0637\u0627\u0642',
        downtimeInformation: '\u0645\u0639\u0644\u0648\u0645\u0627\u062a \u0627\u0644\u062a\u0648\u0642\u0641',
        metadata: '\u0628\u064a\u0627\u0646\u0627\u062a \u0625\u0636\u0627\u0641\u064a\u0629',
        save: '\u062d\u0641\u0638',
        saveAndView: '\u062d\u0641\u0638 \u0648\u0639\u0631\u0636',
        cancel: '\u0625\u0644\u063a\u0627\u0621',
        resetForm: '\u0625\u0639\u0627\u062f\u0629 \u062a\u0639\u064a\u064a\u0646',
        backToList: '\u0627\u0644\u0639\u0648\u062f\u0629 \u0644\u0644\u0642\u0627\u0626\u0645\u0629',
        viewDetails: '\u0639\u0631\u0636 \u0627\u0644\u062a\u0641\u0627\u0635\u064a\u0644',
        recordCreated: '\u062a\u0645 \u0625\u0646\u0634\u0627\u0621 \u0627\u0644\u0633\u062c\u0644 \u0628\u0646\u062c\u0627\u062d',
        recordUpdated: '\u062a\u0645 \u062a\u062d\u062f\u064a\u062b \u0627\u0644\u0633\u062c\u0644 \u0628\u0646\u062c\u0627\u062d',
        createFailed: '\u0641\u0634\u0644 \u0625\u0646\u0634\u0627\u0621 \u0627\u0644\u0633\u062c\u0644',
        updateFailed: '\u0641\u0634\u0644 \u062a\u062d\u062f\u064a\u062b \u0627\u0644\u0633\u062c\u0644',
        loadFailed: '\u0641\u0634\u0644 \u062a\u062d\u0645\u064a\u0644 \u0627\u0644\u0633\u062c\u0644',
        notFound: '\u0627\u0644\u0633\u062c\u0644 \u063a\u064a\u0631 \u0645\u0648\u062c\u0648\u062f',
        readOnlyRecord: '\u0647\u0630\u0627 \u0627\u0644\u0633\u062c\u0644 \u0642\u0627\u0628\u0644 \u0644\u0644\u0642\u0631\u0627\u0621\u0629 \u0641\u0642\u0637 \u0628\u0633\u0628\u0628 \u062d\u0627\u0644\u062a\u0647 \u0627\u0644\u062d\u0627\u0644\u064a\u0629',
        completedRecordReadOnly: '\u0627\u0644\u0633\u062c\u0644\u0627\u062a \u0627\u0644\u0645\u0643\u062a\u0645\u0644\u0629 \u0644\u0627 \u064a\u0645\u0643\u0646 \u062a\u062d\u0631\u064a\u0631\u0647\u0627',
        cancelledRecordReadOnly: '\u0627\u0644\u0633\u062c\u0644\u0627\u062a \u0627\u0644\u0645\u0644\u063a\u064a\u0629 \u0644\u0627 \u064a\u0645\u0643\u0646 \u062a\u062d\u0631\u064a\u0631\u0647\u0627',
        draftOnlyEditable: '\u064a\u0645\u0643\u0646 \u062a\u062d\u0631\u064a\u0631 \u0627\u0644\u0645\u0633\u0648\u062f\u0627\u062a \u0641\u0642\u0637',
        selectProductCategory: '\u0627\u062e\u062a\u0631 \u062a\u0635\u0646\u064a\u0641 \u0645\u0646\u062a\u062c',
        selectMachineCategory: '\u0627\u062e\u062a\u0631 \u062a\u0635\u0646\u064a\u0641 \u0645\u0627\u0643\u064a\u0646\u0629',
        selectMachine: '\u0627\u062e\u062a\u0631 \u0645\u0627\u0643\u064a\u0646\u0629',
        selectWarehouse: '\u0627\u062e\u062a\u0631 \u0645\u062e\u0632\u0646',
        selectWarehouseLocation: '\u0627\u062e\u062a\u0631 \u0645\u0643\u0627\u0646\u0627',
        selectCompany: '\u0627\u062e\u062a\u0631 \u0634\u0631\u0643\u0629',
        selectBranch: '\u0627\u062e\u062a\u0631 \u0641\u0631\u0639',
        selectDepartment: '\u0627\u062e\u062a\u0631 \u0642\u0633\u0645',
        selectUser: '\u0627\u062e\u062a\u0631 \u0645\u0633\u062a\u062e\u062f\u0645',
        selectMaintenanceRequest: '\u0627\u062e\u062a\u0631 \u0637\u0644\u0628 \u0635\u064a\u0627\u0646\u0629',
        generatedAfterSave: '\u064a\u062a\u0645 \u0627\u0644\u062a\u0648\u0644\u064a\u062f \u0628\u0639\u062f \u0627\u0644\u062d\u0641\u0638',
        linkedWorkflows: '\u0633\u064a\u0627\u0642 \u0627\u0644\u0639\u0645\u0644 \u0627\u0644\u0645\u0631\u062a\u0628\u0637',
        goToExecute: '\u062a\u0646\u0641\u064a\u0630 \u0627\u0644\u062c\u0631\u062f',
        goToReview: '\u0645\u0631\u0627\u062c\u0639\u0629',
        goToApprove: '\u0627\u0639\u062a\u0645\u0627\u062f',
        goToAdjustment: '\u062a\u0633\u0648\u064a\u0629',
        goToAssign: '\u062a\u0639\u064a\u064a\u0646 \u0641\u0646\u064a',
        goToParts: '\u0642\u0637\u0639 \u063a\u064a\u0627\u0631',
        goToCost: '\u0627\u0644\u062a\u0643\u0627\u0644\u064a\u0641',
        goToQr: '\u0631\u0645\u0632 QR',
        noPermission: '\u0644\u064a\u0633 \u0644\u062f\u064a\u0643 \u0627\u0644\u0635\u0644\u0627\u062d\u064a\u0629 \u0644\u062a\u0646\u0641\u064a\u0630 \u0647\u0630\u0647 \u0627\u0644\u0639\u0645\u0644\u064a\u0629',
        requiredField: '\u0647\u0630\u0627 \u0627\u0644\u062d\u0642\u0644 \u0645\u0637\u0644\u0648\u0628',
        invalidValue: '\u0642\u064a\u0645\u0629 \u063a\u064a\u0631 \u0635\u0627\u0644\u062d\u0629',
        confirmLeaveUnsaved: '\u0644\u062f\u064a\u0643 \u062a\u063a\u064a\u064a\u0631\u0627\u062a \u063a\u064a\u0631 \u0645\u062d\u0641\u0648\u0638\u0629. \u0647\u0644 \u0623\u0646\u062a \u0645\u062a\u0623\u0643\u062f \u0645\u0646 \u0627\u0644\u0631\u063a\u0628\u0629 \u0641\u064a \u0627\u0644\u0645\u063a\u0627\u062f\u0631\u0629\u061f',
        unsavedChanges: '\u062a\u063a\u064a\u064a\u0631\u0627\u062a \u063a\u064a\u0631 \u0645\u062d\u0641\u0648\u0638\u0629',
        operationSucceeded: '\u0627\u0643\u062a\u0645\u0644\u062a \u0627\u0644\u0639\u0645\u0644\u064a\u0629 \u0628\u0646\u062c\u0627\u062d',
        operationFailed: '\u0641\u0634\u0644\u062a \u0627\u0644\u0639\u0645\u0644\u064a\u0629',
    }
};

export default validation;
