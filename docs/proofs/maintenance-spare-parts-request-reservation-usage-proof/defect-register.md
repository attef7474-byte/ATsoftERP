...
1. **Smoke test login failure in non-interactive mode**: The smoke test script prompts for admin password via `Read-Host` when no password is passed. When run without `-Password` flag, it reads empty string causing "password must be longer than or equal to 6 characters" error. Fixed by passing `-Password <REDACTED>`.
