...
async function api(method: string, path: string, body?: any, useToken = true) {
...
async function main() {
  ...
  await assert('B0  fetch machines', async () => {
    ...
    machineIds = d.data.map((m: any) => m.id);
    machineId = machineIds[0];
    b0pass = true;
  });
  if (!b0pass) { console.log('  SKIP  B7-B19  — no machines available'); return; }

  await assert('B0a  cancel existing OPEN preventive requests', async () => {
    for (const mid of machineIds) {
      const r = await api('GET', `/maintenance/requests?machineId=${mid}&type=PREVENTIVE&status=OPEN&limit=5`);
      const d = await r.json();
  ...
  await assert('B10  due schedules endpoint returns 200', async () => {
    const r = await api('GET', '/maintenance/preventive/generate-due-tasks');
  ...
  await assert('B12  generated request has auto code', async () => {
    const r = await api('GET', `/maintenance/requests/${preventiveReqId}`);
    const d = await r.json();
    if (!d.code || !/^MR-/.test(d.code)) throw new Error(`bad code: ${d.code}`);
  });

  await assert('B13  generated request links to schedule', async () => {
    const r = await api('GET', `/maintenance/requests/${preventiveReqId}`);
    const d = await r.json();
    if (d.scheduleId !== scheduleId) throw new Error(`expected ${scheduleId} got ${d.scheduleId}`);
  });
  ...
  await assert('C30  reopen from CLOSED works', async () => {
    const r = await api('PATCH', `/maintenance/requests/${tempReqId}/reopen`);
    if (r.status !== 200) throw new Error(`status=${r.status}`);
  });

  await assert('C31  cancel from OPEN works', async () => {
    const r = await api('PATCH', `/maintenance/requests/${tempReqId}/cancel`);
    if (r.status !== 200) throw new Error(`status=${r.status}`);
  });
  ...
  if (checklistExists) {
    ...
    if (checklistExecId) {
      ...
      await assert('D33b  fetch checklist items', async () => {
        ...
        const d = await r.json();
        if (r.status === 200 && d.data?.length) itemIds = d.data.map((i: any) => i.id);
      });
  ...
  await assert('E41  emergency request has auto code', async () => {
  ...
  await assert('F60  recent emergency list returns real record', async () => {
    ...
    const d = await r.json();
    if (r.status !== 200 || !Array.isArray(d.data)) throw new Error(`status=${r.status}`);
    if (d.data.length > 0 && !d.data.some((x: any) => x.isEmergency === true)) throw new Error('no emergency in list');
  });
  ...
  await assert('G63  edit detail endpoint returns full data for prefill', async () => {
    const r = await api('GET', `/maintenance/requests/${emergencyReqId}`);
    const d = await r.json();
    if (!d.id || !d.title || !d.machineId || !d.code || !d.status) throw new Error('incomplete data');
  });

  await assert('G64  code immutability enforced  (code field read-only in PATCH)', async () => {
    const r = await api('GET', `/maintenance/requests/${emergencyReqId}`);
    const d = await r.json();
    if (!/^MR-/.test(d.code)) throw new Error('code missing');
  });
  ...
  await assert('G65  Number Sequence increments on create (unique MR- codes)', async () => {
    ...
    const d = await r.json();
    const codes = d.data.map((x: any) => x.code).filter(Boolean);
  ...
  await assert('G66  Number Sequence does NOT increment on edit/start/complete (code unchanged)', async () => {
    const r = await api('GET', `/maintenance/requests/${emergencyReqId}`);
    const d = await r.json();
    if (!/^MR-/.test(d.code)) throw new Error('code changed or missing');
  });
  ...
  await assert('G68  isEmergency filter works (only emergencies returned)', async () => {
    ...
    const d = await r.json();
    if (r.status !== 200) throw new Error(`status=${r.status}`);
    if (!d.data.every((x: any) => x.isEmergency === true)) throw new Error('non-emergency in filtered list');
  });
  ...
  await assert('G69  non-emergency requests still list correctly', async () => {
    ...
    const d = await r.json();
    if (r.status !== 200) throw new Error(`status=${r.status}`);
    if (!d.data.every((x: any) => x.isEmergency === false || x.isEmergency === null)) throw new Error('emergency in non-emerg list');
  });
